...
func TestFile_isValid(t *testing.T) {
	testCases := []struct {
		...
		{testName: "is invalid when points to a ancestor directory", filename: "../../test.txt", expected: false},
		{testName: "is invalid when points to a ancestor directory with complex path", filename: "this/is/the/./directory/../../../../../../test.txt", expected: false},
